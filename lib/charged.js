/**
 * Chargify High-Level API - inspired from Charged (https://github.com/chjj/Charged)
 *
 * Contains some minor changes to the authentication method + a requests logger
 */
...
function Charged(options) {
  ...
  this.apiKey = options.key || options.apiKey || options.api_key;
  this.siteKey = options.siteKey || options.site_key;
  ...
  if (!this.subdomain || !this.apiKey) {
    throw new
    Error('Options `subdomain` and `api_key` are required for Charged.');
  }
}
...
Charged.prototype.migrateSubscription = function(name, options, callback) {
  ...
  return this.post(path, options, function(err, result) {
    return err ? callback(err) : callback(null, result.subscription);
  }, 'migration');
};
...
Charged.prototype.getProductFamilyComponent =
  Charged.prototype.getFamilyComponent = function(name, comp, callback) {
    ...
    var path = '/product_families/' + escape(name) + '/components/' + escape(comp);
    return this.get(path, callback, 'component');
};
...
Charged.prototype.createProductFamilyComponent =
  Charged.prototype.createFamilyComponent = function(name, kind, options, callback) {
    ...
    if (kind !== 'metered_components' && kind !== 'quantity_based_components' && kind !== 'on_off_components') {
...
Charged.prototype.getSubscriptionComponent = function(sub, comp, callback) {
  ...
  var path = '/subscriptions/' + escape(sub) + '/components/' + escape(comp);
  return this.get(path, callback, 'component');
};
...
Charged.prototype.getSubscriptionUsage =
  Charged.prototype.listSubscriptionUsage = function(sub, comp, options, callback) {
    ...
    var path = '/subscriptions/' + escape(sub) + '/components/' + escape(comp) + '/usages';
...
Charged.prototype.createSubscriptionQuantity = function(options, callback) {
  if (!options.components || !options.components.length || options.components[0].allocated_quanity == null) {
...
Charged.prototype.addSubscriptionCoupon = function(sub, name, callback) {
  var path = '/subscriptions/' + escape(sub) + '/add_coupon?code=' + escape(name);
  return this.post(path, callback, 'coupon');
};

Charged.prototype.removeSubscriptionCoupon = function(sub, name, callback) {
  var path = '/subscriptions/' + escape(sub) + '/remove_coupon?code=' + escape(name);
...
Charged.prototype._listMetadata = function(resource, name, options, callback) {
  if (!callback) {
    callback = options;
    options = null;
  }
  var path = '/' + escape(resource) + '/' + escape(name) + '/metadata';
  return this.get(path, options, callback, 'metadata');
};

Charged.prototype._getMetadata = function(resource, name, id, options, callback) {
  if (!callback) {
    callback = options;
    options = null;
  }
  var path = '/' + escape(resource) + '/' + escape(name) + '/' + escape(id) + '/metadata';
  return this.get(path, options, callback, 'metadata');
};

Charged.prototype._createMetadata = function(resource, name, id, options, callback) {
  if (!callback) {
    callback = options;
    options = null;
  }
...
Charged.prototype._updateMetadata = function(resource, name, id, options, callback) {
  if (!callback) {
    callback = options;
    options = null;
  }
...
Charged.prototype._removeMetadata = function(resource, name, id, options, callback) {
  if (!callback) {
    callback = options;
    options = null;
  }
  ...
  delete options.name;
  var path = '/' + escape(resource) + '/' + escape(name) + '/' + escape(id) + '/metadata?name=' + escape(name);
...
['subscription', 'customer', 'component'].forEach(function(resource) {
  ['list', 'get', 'create', 'update', 'remove'].forEach(function(action) {
    var name = '_' + action + 'Metadata',
      plural = resource + 's',
      method;

    method = action + resource[0].toUpperCase() + resource.substring(1) + 'Metadata';
...
Charged.prototype.removeMetafields = function(resource, options, callback) {
  ...
  var path = '/' + escape(resource) + '/metafields?name=' + escape(field);
...
Charged.prototype.request = function(options, callback) {
  ...
  options = {
        method: method,
        uri: url.format(_(url.parse(path)).defaults({
            protocol: 'https',
            host: this.host,
            auth: this.apiKey + ':x'
        })),
  ...
  return request(options, function(err, res, body) {
    if (err) {
      err = 'No response from chargify' + (err.code ? ': ' + err.code : '') + '.';
    ...
    try {
      ...
      return callback(new Error(
        'Status code: ' + res.statusCode + '.' + ' The JSON returned from chargify' + ' is failing to parse.' + ' Charged may be having a problem.'));
    }
...
Charged.prototype.getHostedPageToken = function(shortname, id) {
  if (!id) {
    if (!shortname) {
      return null;
    }
  ...
  return sha1(shortname + '--' + id + '--' + this.siteKey).slice(0, 10);
...
Charged.prototype.hostedPage = function(shortname, id) {
  ...
  return 'https://' + this.subdomain + '.chargify.com/' + escape(shortname) + '/' + escape(id) + '/' + escape(this.getHostedPageToken(shortname, id));
};

Charged.prototype.signupPage = function(product) {
  return 'https://' + this.subdomain + '.chargify.com/h/' + escape(product) + '/subscriptions/new';
};

Charged.prototype.getRelevantCustomerDetails = function(subscriberDetails, customerDataFromRemoteProvider) {
        //subscriberDetails contains the list of subscriptions
        if ( typeof subscriberDetails === "object") {
                for (var item in subscriberDetails) {
                        var customerID = subscriberDetails[item].customer.reference;
                        var billingStatusRaw = subscriberDetails[item].state;
      var currentBillingCycleStartDate = subscriberDetails[item].current_period_started_at;
                        var billingStatus = translateChargedSubscriptionState(billingStatusRaw);
                        customerDataFromRemoteProvider[item] = {"customerID":customerID , "billingStatus": billingStatus , "currentBillingCycleStartDate": currentBillingCycleStartDate, "billingStatusRaw": billingStatusRaw};
                }
        }
        return customerDataFromRemoteProvider;
};

translateChargedSubscriptionState = function(billingStatusRaw) {
  var billingStatus;

        switch (billingStatusRaw) {
        case "trailing":
        billingStatus = "liveState";
        break;
        case "assessing":
        billingStatus = "inditerminateState";
        break;
        case "active":
        billingStatus = "liveState";
        break;
        case "soft_failure":
        billingStatus = "problemState-softFailure";
        break;
        case "past_due":
        billingStatus = "problemState-pastDue";
        break;
        case "unpaid":
        billingStatus = "problemState-unpaid";
        break;
        case "canceled":
        billingStatus = "endoflifeState-canceled";
        break;
        case "expired":
        billingStatus = "endoflifeState-expired";
        break;
        case "suspended":
        billingStatus = "endoflifeState-suspended";
        break;
        case "trial_ended":
        billingStatus = "endoflifeState-trial_ended";
        break;
        default:
        billingStatus = "indeterminateState-unknown";
        // log an error
        }
...
function escape(str) {
  ...
  try {
    if ((type === 'string' && str.length) || (type === 'number' && isFinite(str))) {
